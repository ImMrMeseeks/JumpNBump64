...
//		//outportb(0x3c9, fade_pal[c1]);
	...
	end_loop_flag = new_game_flag = 0; //= fade_flag = 0;
	...
	while (1) {
		...
		for (c1 = 0; c1 < 4; c1++) {
			//Control if starting new game
			if (end_loop_flag == 1 && new_game_flag == 1) {

				//add player
			if ( (player[c1].x >> 16) >	(logStart + c1 * 2) ) {		//(player[c1].x >> 16) >	(165 + c1 * 2)
	  	  ...
	  	  player[c1].y += player[c1].y_add;
				if ( (player[c1].x >> 16) <= (logStart + c1 * 2) || (player[c1].x >> 16) >= (logEnd + c1 * 2) ) {	//(165 + c1 * 2) || (player[c1].x >> 16) >= (208 + c1 * 2) )
				...
				if ( (player[c1].y >> 16) >	(138 + c1 * 2) ) {
 	    		if ( (player[c1].x >> 16) > (logStart + c1 * 2) && (player[c1].x >> 16) < (logMid + c1 * 2) ) {		//(165 + c1 * 2) && (player[c1].x >> 16) < (190 + c1 * 2) ) {
   			  	player[c1].x = (logStart + c1 * 2) << 16;	//(165L + c1 * 2) << 16;
 			  	 	player[c1].x_add = 0;
		      }
 	    		if ( (player[c1].x >> 16) > (logMid + c1 * 2) && (player[c1].x >> 16) < (logEnd + c1 * 2) ) {		//(190 + c1 * 2) && (player[c1].x >> 16) < (208 + c1 * 2) ) {
   			  	player[c1].x = (logEnd + c1 * 2) << 16;
 			  	 	player[c1].x_add = 0;
		      }
				}
			}
			//Normal control not starting new level
			else {
		   	if ( (c1 == 0 && key_pressed(KEY_PL1_LEFT) == 1 && key_pressed(KEY_PL1_RIGHT) == 1) || (c1 == 1 && key_pressed(KEY_PL2_LEFT) == 1 && key_pressed(KEY_PL2_RIGHT) == 1) || (c1 == 2 && joy.x < -512 && joy.x > 512) || (c1 == 3 && mouse.but1 == 1 && mouse.but2 == 1) ) {
					if (player[c1].direction == 1) {
						if ( (player[c1].x >> 16) <= (logStart + c1 * 2) || (player[c1].x >> 16) >= (logEnd + c1 * 2) ) {
  	  				if (player[c1].x_add > 0) {
 	   						player[c1].x_add -= 16384;
								if ( (player[c1].y >> 16) >= (160 + c1 * 2) )
...
//  	  	if ( (c1 == 0 && ( (key_pressed(KEY_PL1_LEFT) == 0 && key_pressed(KEY_PL1_RIGHT) == 0) || (key_pressed(KEY_PL1_LEFT) == 1 && key_pressed(KEY_PL1_RIGHT) == 1) ) ) || (c1 == 1 && ( (key_pressed(KEY_PL2_LEFT) == 0 && key_pressed(KEY_PL2_RIGHT) == 0) || (key_pressed(KEY_PL2_LEFT) == 1 && key_pressed(KEY_PL2_RIGHT) == 1) ) ) || (c1 == 2 && ( (joy.x >= -512 && joy.x <= 512) || (joy.x < -512 && joy.x > 512) ) ) || (c1 == 3 && ( (mouse.but1 == 0 && mouse.but2 == 0) || (mouse.but1 == 1 && mouse.but2 == 1) ) ) ) {
					...
					if ( ( (player[c1].x >> 16) <= (logStart + c1 * 2) || (player[c1].x >> 16) >= (logEnd + c1 * 2) ) && (player[c1].y >> 16) >= (160 + c1 * 2) ) {	//( ( (player[c1].x >> 16) <= (165 + c1 * 2) || (player[c1].x >> 16) >= (208 + c1 * 2) ) && (player[c1].y >> 16) >= (160 + c1 * 2) ) {
	    	 		if (player[c1].x_add < 0) {
 	  		  		player[c1].x_add += 16384;
  				    if (player[c1].x_add > 0)
	 	 		  	  	player[c1].x_add = 0;
					...
					if ( ( ( (player[c1].x >> 16) > (logStart + c1 * 2) && (player[c1].x >> 16) < (logEnd + c1 * 2) ) && (player[c1].y >> 16) >= (138 + c1 * 2) ) ) {	//( ( ( (player[c1].x >> 16) > (165 + c1 * 2) && (player[c1].x >> 16) < (208 + c1 * 2) ) && (player[c1].y >> 16) >= (138 + c1 * 2) ) ) {
	    	 		if (player[c1].x_add < 0) {
 	  		  		player[c1].x_add += 16384;
  				    if (player[c1].x_add > 0)
	 	 		  	  	player[c1].x_add = 0;
  	      ...
  	      if (player[c1].anim == 1) {
        	  ...
        	  player[c1].frame_tick = 0;
	   		...
	   		if (player[c1].jump_ready == 1 && ( (c1 == 0 && key_pressed(KEY_PL1_JUMP) == 1) || (c1 == 1 && key_pressed(KEY_PL2_JUMP) == 1) || (c1 == 2 && joy.but1 == 1)  || (c1 == 3 && ( (main_info.num_mouse_buttons == 3 && mouse.but3 == 1) || (main_info.num_mouse_buttons == 2 && mouse.but1 == 1 && mouse.but2 == 1) ) ) ) ) {
					if ( (player[c1].x >> 16) <= (logStart + c1 * 2) || (player[c1].x >> 16) >= (logEnd + c1 * 2) ) { //(player[c1].x >> 16) <= (165 + c1 * 2) || (player[c1].x >> 16) >= (208 + c1 * 2) ) {
						if ( (player[c1].y >> 16) >= (160 + c1 * 2) ) {
			...
			//check for starting level/leaving menu
	 	    if ( (player[c1].x >> 16) > 320) {	 //> 400) {
					end_loop_flag = 1;
					new_game_flag = 1;
					memset(menu_pal, 0, 768);
					//mod_fade_direction = 0;
  			}
				//checks if player is up against log 120 to 163/ 165 to 208
				if ( (player[c1].y >> 16) >	(138 + c1 * 2) ) {
 	    			if ( (player[c1].x >> 16) > (logStart + c1 * 2) && (player[c1].x >> 16) < (logMid + c1 * 2) ) { //(player[c1].x >> 16) > (165 + c1 * 2) && (player[c1].x >> 16) < (190 + c1 * 2) ) {
   			  			player[c1].x = (logStart + c1 * 2) << 16; //(165L + c1 * 2) << 16;
 			  	 		player[c1].x_add = 0;
					}
 	    			if ( (player[c1].x >> 16) > (logMid + c1 * 2) && (player[c1].x >> 16) < (logEnd + c1 * 2) ) { //(player[c1].x >> 16) > (190 + c1 * 2) && (player[c1].x >> 16) < (208 + c1 * 2) ) {
   			  			player[c1].x = (logEnd + c1 * 2) << 16; //(208L + c1 * 2) << 16;
 			  	 		player[c1].x_add = 0;
					}
				}
			}
		...
		for (c1 = 3; c1 >= 0; c1--) {
			add_pob(main_info.draw_page, (player[c1].x >> 16), (player[c1].y >> 16), player[c1].image + c1 * 18, rabbit_gobs); //rabbit_gobs)
			//sprintf(dirText, "c1: %i, x: %i, y: %i, enabled: %i", c1, (player[c1].x >> 16), (player[c1].y >> 16), player[c1].enabled);
			//graphics_draw_text(_dc, 5, c1 * 10, dirText);
		}
		...
		//leave menu to enter game or quit application
		if (fade_dir == 2 && fade_flag == 0 && end_loop_flag == 1) {
			menu_deinit();
			if (new_game_flag == 1) {
				return 0;
			}			
			...
			switch (fade_dir) {
				...
				case 0:
					if (fade_count < 30) {
						...
						//hack fade in
						//if(buffer) for (c1 = 0; c1 < 62000; c1++) *(buffer + c1) &= (unsigned short)((fade_count << 11) | (fade_count << 6) | (fade_count << 1) | 1);
					 ...
					 // memset( (char *)(0xa0000 + 32768 + 220 * 100 + __djgpp_conventional_base), 0, 2000);
						...
						//put_text(0, 160, 220, message[cur_message], 2); //200, 220
						//put_text(1, 160, 220, message[cur_message], 2);
						fade_dir = 1;
						fade_count = 0;
						fade_tick = 0;
					}
				...
				case 1:
					if (fade_count < 30) { //100) {
						...
						//hack fade out
						char trans = 31 - fade_count;
						for (c1 = 0; c1 < 62000; c1++) *(buffer + c1) &= ((trans << 11) | (trans << 6) | (trans << 1) | 1);

						fade_count++;
					}
					else {

						fade_dir = 2;	//fade_dir = 0;
						fade_count = 0;
						fade_tick = 0;
					}
					break;
		...
		//graphics_draw_text(_dc, 20, 220, message[cur_message]);
		put_text(0, 20, 220, message[cur_message], 0); //160 //200

		//debug
		//sprintf(dirText, "fade_dir: %i, fade_flag: %i, endLFlag; %i", fade_dir, fade_flag, end_loop_flag);
		//graphics_draw_text(_dc, 5, 50, dirText);
...
char menu_init(void) {
 ...
 //		//outportb(0x3c9, 0);
	...
	for (c1 = 0; c1 < 40; c1++) {
		sprintf(spritepath, "/Menu%i.sprite", c1 + 1);
	...
	//background_pic = SprMenu;
	//handle = dat_open("/menu1.sprite", datfile_name, "rb");

	//	//load menu screen
 // if ((handle = dat_open("/menu.sprite", datfile_name, "rb") ) == 0) { //("menu.pcx", datfile_name, "rb")
